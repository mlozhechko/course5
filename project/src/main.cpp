...
bool program_options(int argc, char** argv) {
    ...
    if (vm.count("file") && vm.count("destination")) {
        ...
        std::cout << "Error! Source filename and destination filename must be specified"
                  << std::endl;
        std::cout << desc << std::endl;
        return is_ready;
    }
...
int main(int argc, char** argv) {
    ...
    std::cout << "Defined grid resolution: " << config.resolution_x << "x" << config.resolution_y
              << std::endl;
    ...
    std::cout << "Initial system angle around y: " << config.system_initial_angle_around_y << " Pi"
              << std::endl;
    std::cout << "Limit alpha value: " << config.limit_alpha_value << std::endl;
    omp_set_num_threads(config.threads / 2);

    auto t1 = timestamp();
    const double make_perpendicular_to_y_angle =
        -config.system_initial_angle_around_y * PI + PI / 2.;
    ...
    auto roche_lobe = object3d_roche_lobe{
        {ACC_X0, ACC_Y0, ACC_Z0}, L, config.donor_angle * PI, M_ACC, M_DONOR, OMEGA};
    ...
    acc_t.join();
    auto t2 = timestamp();
    std::cout << "Loading data with VTK lib and other preparations completed in "
    ...
    t1 = timestamp();
    plane base_plane{
        config.resolution_x, config.resolution_y, {acc_disk, roche_lobe, acc_sphere}, domain};
    base_plane.find_intersections();
    object2d result = base_plane.trace_rays(tetra_value::alpha, tetra_value::Q);
    t2 = timestamp();
    std::cout << "Ray-tracing completed in "
              << std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count() << " ms. "
              << std::endl;
