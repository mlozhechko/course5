...
void object3d_base::read_vtk_file(const std::string& filename, const std::vector<std::string>& scalar_labels) {
...
#pragma omp parallel for default(none) shared(_data) reduction(max: x_max, y_max)        \
reduction(min: x_min, y_min) schedule(dynamic, 8)
